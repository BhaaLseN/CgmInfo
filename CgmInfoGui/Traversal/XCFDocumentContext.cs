...
public class XCFDocumentContext
{
    ...
    public void AddAPSAttribute(ApplicationStructureAttribute applicationStructureAttribute)
    {
        if (_lastElement != null)
        {
            ...
            {
                // [WebCGM20-XCF 4.3.8]
                object[] allValues = applicationStructureAttribute.DataRecord.Elements.SelectMany(el => el.Values).ToArray();
        ...
        switch (applicationStructureAttribute.AttributeType.ToUpperInvariant())
        {
            ...
            case "INTERACTIVITY":
                if (allValues.Length == 1)
                    return Convert.ToString(allValues[0]) ?? string.Empty;
                else
                    return "#INVALID";
            case "REGION":
            case "VIEWCONTEXT":
    ...
    private static string[] GetSupportedAttributes(string apsName) =>
        // NOTE: apsid is derived from the actual APS beginning; not from an APS attribute (hence omitted here)
        apsName.ToUpperInvariant() switch
        {
            // [WebCGM20-XCF 4.3.4]
            "LAYER" => ["layerdesc", "visibility", "interactivity"],
            // [WebCGM20-XCF 4.3.5]
            "GROBJECT" or "PARA" or "SUBPARA" => ["screentip", "region", "viewcontext", "visibility", "interactivity"],
            _ => [],
        };
}
